...
def read_s16(path):
    with open(path, "rb") as f:
        data = f.read()
    n = len(data) // 2
    return list(struct.unpack("<%dh" % n, data[: n * 2])), len(data)
...
def main():
    ...
    if args.frames is not None:
        ...
        if out_bytes != want:
            failures.append(f"output bytes {out_bytes} != expected {want} "
                            f"({args.frames} x {args.frame_bytes})")
        else:
    ...
    if args.expect_silence:
        ...
        if bad:
            failures.append(f"gain mismatch: {bad}/{n} samples off (max_err={max_err}); "
                            f"first bad idx {first_bad[0]}: in={first_bad[1]} "
                            f"exp={first_bad[2]} got={first_bad[3]}")
        else:
            label = ("byte-exact pass-through" if args.gain == 100 else
                     "mute (all zero)" if args.gain == 0 else
                     f"gain {args.gain}%")
