...
int main(void)
{
	...
	ret = mp_object_set_properties((struct mp_object *)&source, PROP_NUM_BUFS,
	...
	if (mp_bin_add((struct mp_bin *)&pipe,
		       ...
		       (struct mp_element *)&sink, NULL) != 0) {
		LOG_ERR("Failed to add elements to the pipeline");
		return 0;
	}

	if (mp_element_link((struct mp_element *)&source,
			    IF_ENABLED(CONFIG_MP_CAPSFILTER, ((struct mp_element *)&caps_filter,))
			    (struct mp_element *)&gain,
			    (struct mp_element *)&sink, NULL) != 0) {
	...
	if (mp_bus_pop_msg(bus, MP_MESSAGE_ERROR | MP_MESSAGE_EOS, &msg) == 0) {
		switch (msg.type) {
		case MP_MESSAGE_EOS:
			LOG_INF("EOS received from element %d", msg.origin->object.id);
			break;
		case MP_MESSAGE_ERROR:
			LOG_ERR("ERROR received from element %d", msg.origin->object.id);
			break;
		default:
			LOG_WRN("unexpected message (type %u) from element %d", msg.type,
				msg.origin->object.id);
			break;
		}
	}
	 ...
	 */
	mp_element_set_state((struct mp_element *)&pipe, MP_STATE_PAUSED);
	k_msleep(DRAIN_MARGIN_MS);

	if (mp_element_set_state((struct mp_element *)&pipe, MP_STATE_READY) !=
	    MP_STATE_CHANGE_SUCCESS) {
