...
int micro_speech_process_audio(const int16_t *audio_data,
    ...
    /*
     * Return the category, not just a status. Upstream returns 0 on success and
     * logs the label internally, which leaves a caller unable to act on the
     * result at all -- and a pipeline stage has to act on it.
     */
    return category;
}
