...
LOG_MODULE_DECLARE(mp_ipc_loopback_app, LOG_LEVEL_INF);
...
static int mp_test_sink_chainfn(struct mp_pad *pad, struct net_buf *buf, struct net_buf **out_buf)
{
	...
	self->frame_count++;
	LOG_INF("[Pipeline B: mp_test_sink] Received Frame %d: size=%u, timestamp=%u", 
		self->frame_count, buf->len, meta->timestamp);

	/* When we receive 3 frames, post an EOS message to Pipeline B's bus */
