...
BUILD_ASSERT(DT_SAME_NODE(DT_INST_CLOCKS_CTLR_BY_IDX(0, 1), DT_NODELABEL(sai3_mclk1)));
BUILD_ASSERT(DT_INST_PHA_BY_IDX_OR(0, clocks, 1, name, 0U) == 0U);

/*
 * Instantiate the production initializer against a two-provider node, so a
 * change that reintroduces one shared controller for the whole array fails to
 * compile here.
 */
