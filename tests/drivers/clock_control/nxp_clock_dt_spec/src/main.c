...
#define CLOCK_ALT_NODE     DT_NODELABEL(clock_alt)
#define CONSUMER_NODE      DT_NODELABEL(clock_consumer)
#define ALT_CONSUMER_NODE  DT_NODELABEL(clock_alt_consumer)
...
struct fake_clock_data {
	...
	uint32_t set_rate_count;
	int set_rate_result;
	clock_control_subsys_rate_t last_rate;
...
static enum clock_control_status fake_clock_get_status(const struct device *dev,
...
static int fake_clock_set_rate(const struct device *dev, clock_control_subsys_t subsys,
...
static DEVICE_API(clock_control, fake_clock_api) = {
	...
	.get_status = fake_clock_get_status,
	.set_rate = fake_clock_set_rate,
};
...
/*
 * The two situations the helper must keep apart: a provider with no cells,
 * whose subsystem is legitimately zero, and a provider whose cell is named
 * something else, which must not silently become zero.
 * NXP_CLOCK_DT_SPEC_GET_BY_IDX(ALT_CONSUMER_NODE, 0) does not compile - it is
 * the ZERO_OR_COMPILE_ERROR() arm - so the guard is asserted here on the same
 * devicetree predicates the macro branches on.
 */
ZTEST(nxp_clock_dt_spec, test_named_cell_is_required_of_a_celled_provider)
{
	zassert_equal(DT_PHA_HAS_CELL_AT_IDX(CONSUMER_NODE, clocks, 2, name), 0);
	zassert_equal(DT_PHA_NUM_CELLS_BY_IDX(CONSUMER_NODE, clocks, 2), 0);

	zassert_equal(DT_PHA_HAS_CELL_AT_IDX(ALT_CONSUMER_NODE, clocks, 0, name), 0);
	zassert_equal(DT_PHA_NUM_CELLS_BY_IDX(ALT_CONSUMER_NODE, clocks, 0), 1);
	zassert_equal(DT_PHA_BY_IDX(ALT_CONSUMER_NODE, clocks, 0, clkid), 0x44U);
}
...
ZTEST(nxp_clock_dt_spec, test_status_and_rate_wrappers_forward_provider_and_subsystem)
{
	clock_b_data.status = CLOCK_CONTROL_STATUS_OFF;

	zassert_equal(nxp_clock_control_get_status_dt(&indexed_specs[1]), CLOCK_CONTROL_STATUS_OFF);
	zassert_equal(clock_b_data.status_count, 1U);
	zassert_equal(POINTER_TO_UINT(clock_b_data.last_subsys), 0x22U);

	zassert_ok(nxp_clock_control_set_rate_dt(&indexed_specs[0], UINT_TO_POINTER(48000U)));
	zassert_equal(clock_a_data.set_rate_count, 1U);
	zassert_equal(POINTER_TO_UINT(clock_a_data.last_subsys), 0x11U);
	zassert_equal(POINTER_TO_UINT(clock_a_data.last_rate), 48000U);
}

/* Catches removal of the -ENODEV gate on the two wrappers added last. */
ZTEST(nxp_clock_dt_spec, test_status_and_rate_wrappers_reject_unusable_specs)
{
	zassert_equal(nxp_clock_control_get_status_dt(NULL), CLOCK_CONTROL_STATUS_UNKNOWN);
	zassert_equal(nxp_clock_control_get_status_dt(&fallback_spec),
		      CLOCK_CONTROL_STATUS_UNKNOWN);
	zassert_equal(nxp_clock_control_set_rate_dt(NULL, UINT_TO_POINTER(1U)), -ENODEV);
	zassert_equal(nxp_clock_control_set_rate_dt(&fallback_spec, UINT_TO_POINTER(1U)), -ENODEV);
	zassert_equal(clock_a_data.set_rate_count, 0U);
	zassert_equal(clock_b_data.status_count, 0U);
}

/* Catches a wrapper mutation that discards a backend set_rate error. */
ZTEST(nxp_clock_dt_spec, test_rate_wrapper_propagates_backend_error)
{
	clock_a_data.set_rate_result = -ENOTSUP;

	zassert_equal(nxp_clock_control_set_rate_dt(&indexed_specs[0], UINT_TO_POINTER(96000U)),
		      -ENOTSUP);
	zassert_equal(clock_a_data.set_rate_count, 1U);
	zassert_equal(POINTER_TO_UINT(clock_a_data.last_rate), 96000U);
}
