...
static int mp_zaud_gain_set_property(struct mp_object *obj, uint32_t key, const void *val)
{
	...
	switch (key) {
	case PROP_GAIN:
		/* Clamp once on store so gain_percent, gain_fixed, mute and the
		 * get_property readback all stay consistent for out-of-range input.
		 */
		self->gain_percent = CLAMP(*(const int *)val, GAIN_PERCENT_MIN, GAIN_PERCENT_MAX);
