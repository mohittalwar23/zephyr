							...
							struct mpipe_dispatch *query)
{
	...
	/* Silence blocks stay queued until the receiver returns its first block. */
...
static int mpipe_aud_i2s_codec_sink_prime_silence(struct mpipe_aud_i2s_codec_sink *sink)
{
	...
	/* A buffer written after a pause may have left a stale count. */
	sink->count = 0U;
						 ...
						 enum mpipe_state_change transition)
{
	...
	if (transition == MPIPE_STATE_CHANGE_PAUSED_TO_PLAYING &&
		...
		int ret = mpipe_aud_i2s_codec_sink_prime_silence(aud_i2s_codec_sink);

		if (ret != 0) {
			LOG_ERR("Unable to prime synchronized I2S capture clocks: %d", ret);
			return ret;
		}
	}
#endif
