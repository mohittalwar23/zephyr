...
#define WM8960_FS256_RATIO        256U
#define WM8960_RATIO_TENTHS       10U
#define WM8960_SYSCLKDIV_BY_1     0x0000U
#define WM8960_SYSCLKDIV_BY_2     BIT(1)
#define WM8960_DACDIV_SHIFT       3U
#define WM8960_ADCDIV_SHIFT       6U
#define WM8960_DCLKDIV_SHIFT      6U
...
#define WM8960_CLASSD1_SPKOP_EN_L 0x0080 /* Speaker Output Enable - left */
#define WM8960_CLASSD1_SPKOP_EN_R 0x0040 /* Speaker Output Enable - right */
...
#define WM8960_PLL1_OPCLKDIV_MASK 0x01C0 /* Mask for the SYSCLK divider */
...
#define WM8960_US_WAIT_PLL   10000
#define WM8960_US_WAIT_RESET 100000
#define WM8960_US_WAIT_VMID  100000
#define WM8960_US_WAIT_VOLCH 10000
...
static const uint16_t wm8960_default_regs[WM8960_LAST_REG + 1] = {
...
static int wm8960_soft_reset(const struct device *dev)
{
	...
	k_busy_wait(WM8960_US_WAIT_RESET);

	/* Initialise register cache to defaults */
	for (size_t i = 0; i < ARRAY_SIZE(wm8960_default_regs); i++) {
		data->reg_cache[i] = wm8960_default_regs[i];
	}
					 ...
					 bool use_sysdiv)
{
	...
	/* 4. configure PLL */
	pll1_val = (prescale ? 0x0010 : 0x0000) | (plln & 0x000F) | 0x0020;

	ret = wm8960_reg_write(dev, WM8960_PLL1, pll1_val);
	if (ret < 0) {
		return ret;
	}

	ret = wm8960_reg_write(dev, WM8960_PLL2, (pllk >> 16) & 0x00FF);
	if (ret < 0) {
		return ret;
	}

	ret = wm8960_reg_write(dev, WM8960_PLL3, (pllk >> 8) & 0x00FF);
	if (ret < 0) {
		return ret;
	}

	ret = wm8960_reg_write(dev, WM8960_PLL4, pllk & 0x00FF);
	if (ret < 0) {
		return ret;
	}
 ...
 */
static int wm8960_i2s_mclk_calculate(const struct device *dev,
				      struct audio_codec_cfg *cfg)
{
	...
	uint32_t ratio;
	uint32_t remainder;
	uint32_t remainder_div;
	uint32_t target_freq;
	static const int div[] = {2, 3, 4, 6, 8, 11, 12};

	if (cfg->dai_cfg.i2s.format != I2S_FMT_DATA_FORMAT_I2S) {
		return -EINVAL;
	}

	frame_clk_freq = cfg->dai_cfg.i2s.frame_clk_freq;
	if (frame_clk_freq == 0U) {
		LOG_ERR("dai_cfg.i2s.frame_clk_freq is 0");
		return -EINVAL;
	}

	/* Figure 36 Clocking Scheme shows SYSCLK defaults to 256 x Fs. */
	fs256_freq = WM8960_FS256_RATIO * frame_clk_freq;
	ratio = data->mclk_in_freq / fs256_freq;
	remainder = data->mclk_in_freq % fs256_freq;
	remainder_div = (remainder == 0U) ? 0U : (fs256_freq / remainder);

	if ((ratio == 8U || ratio == 11U || ratio == 12U) && (remainder == 0U)) {
		/* do not use pll, only divider can work */
		data->use_pll = 0;
		/* as adc/dac div only support up to 6 */
		ret = wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_SYSCLKDIV,
				  WM8960_SYSCLKDIV_BY_2);
		if (ret < 0) {
			return ret;
		}

		/* SYSCLK is half of sysclk_out */
		data->sysclk_out_freq = fs256_freq * 2U;
		return 0;
	}

	if ((ratio <= 4U || ratio == 6U) && (remainder == 0U)) {
	...
	if (remainder_div == 2U) {
		...
		if (ratio == 11U) {
			/* as adc/dac div only support up to 6 */
			ret = wm8960_reg_update(dev, WM8960_CLOCK1,
						WM8960_CLOCK1_SYSCLKDIV,
						WM8960_SYSCLKDIV_BY_2);
			if (ret < 0) {
				return ret;
			...
			/* SYSCLK is half of sysclk_out */
			data->sysclk_out_freq = data->mclk_in_freq * 2U;
			data->use_pll = 0;
			return 0;
		}
	}

	for (size_t i = 0; i < ARRAY_SIZE(div); i++) {
		target_freq = fs256_freq * div[i] / 2U;
		...
		if (ret != 0) {
			LOG_WRN("target clock %d can not use PLL at %d",
				target_freq, data->mclk_in_freq);
		} else {
			data->use_pll = 1;
			data->sysclk_out_freq = target_freq;
			return 0;
		}
		/* try not use sysdiv */
		ret = wm8960_pll_config_precise(dev, data->mclk_in_freq,
...
/* set the pll_out_freq based on requirements */
static int wm8960_configure_pll(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	ret = wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_CLKSEL, 0);
	if (ret < 0) {
		return ret;
	}

	ret = wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_SYSCLKDIV, 0);
	if (ret < 0) {
		return ret;
	}

	/* power on pll  */
	ret = wm8960_reg_update(dev, WM8960_POWER2, WM8960_POWER2_PLL_EN, WM8960_POWER2_PLL_EN);
	if (ret < 0) {
		return ret;
	}

	/* wait PLL to stabilise */
	k_busy_wait(WM8960_US_WAIT_PLL);
	...
	if ((cfg->dai_cfg.i2s.options & I2S_OPT_FRAME_CLK_TARGET) == I2S_OPT_FRAME_CLK_CONTROLLER) {
		...
		if (config->clk_out) {
			/* OPCLKDIV */
			ret = wm8960_reg_update(dev, WM8960_PLL1, WM8960_PLL1_OPCLKDIV_MASK, 0);
			if (ret < 0) {
				return ret;
			}
		...
		/* GPIOSEL */
		ret = wm8960_reg_update(dev, WM8960_ADDCTL4, WM8960_ADDCTL4_GPIOSEL,
				  config->gpio1_sel << 4);
		if (ret < 0) {
			return ret;
		}
		...
		/* keeps in GPIO mode */
		ret = wm8960_reg_update(dev, WM8960_ADDCTL4, WM8960_ADDCTL4_GPIOSEL, 0);
		if (ret < 0) {
			return ret;
		}
	}
...
static int wm8960_route_output(const struct device *dev, audio_channel_t channel, uint32_t output)
{
	...
	case AUDIO_CHANNEL_FRONT_LEFT:
		...
		v_classd1_enable = WM8960_CLASSD1_SPKOP_EN_L;
		m_classd1 = WM8960_CLASSD1_SPKOP_EN_L;
		break;
	...
	case AUDIO_CHANNEL_FRONT_RIGHT:
		...
		v_classd1_enable = WM8960_CLASSD1_SPKOP_EN_R;
		m_classd1 = WM8960_CLASSD1_SPKOP_EN_R;
		break;
...
static int wm8960_power_up(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	ret = wm8960_reg_update(dev, WM8960_POWER1,
	...
	k_busy_wait(WM8960_US_WAIT_VMID);
...
static int wm8960_audio_fmt_config(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	struct wm8960_data *data = dev->data;
	uint32_t fs256 = WM8960_FS256_RATIO * dai_cfg->i2s.frame_clk_freq;
	uint32_t bclk =
	...
	ret = wm8960_reg_update(dev, WM8960_CLOCK2,
			  WM8960_CLOCK2_BCLKDIV | WM8960_CLOCK2_DCLKDIV,
			  val | (WM8960_CLOCK2_DCLKDIV_DEFAULT << WM8960_DCLKDIV_SHIFT));
	if (ret < 0) {
		return ret;
	}
	...
	/* set adc div */
	ret = wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_ADCDIV,
			  val << WM8960_ADCDIV_SHIFT);
	if (ret < 0) {
		return ret;
	}

	/* set dac div */
	ret = wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_DACDIV,
			  val << WM8960_DACDIV_SHIFT);
	if (ret < 0) {
		return ret;
	}
...
static int wm8960_set_mute(const struct device *dev, bool mute)
{
	...
	if (mute) {
		...
		ret = wm8960_reg_update(dev, WM8960_DACCTL1, WM8960_DACCTL1_DACMU, 0);
		if (ret < 0) {
			return ret;
		}
	}
...
static int wm8960_set_input_volume(const struct device *dev, uint8_t l, uint8_t r, bool store)
{
	...
	/* Clamp volume to supported range */
	...
	k_busy_wait(WM8960_US_WAIT_VOLCH);
...
static int wm8960_set_output_volume(const struct device *dev, uint8_t l, uint8_t r, bool store)
{
	...
	/* Clamp volume to supported range */
	l = CLAMP(l, 0U, WM8960_MAX_OUT_VOLUME);
	r = CLAMP(r, 0U, WM8960_MAX_OUT_VOLUME);

	/*
	 * Left and right DAC gains kept at unity (0 dB), volume control done by
	 * changing output gains of respective blocks.
	 */
	ret = wm8960_reg_write(dev, WM8960_LDAC, 0xFF);
	if (ret < 0) {
		return ret;
	}
	ret = wm8960_reg_write(dev, WM8960_RDAC, 0xFF);
	if (ret < 0) {
		return ret;
	}

	/* Headphone output volume */
	ret = wm8960_reg_write(dev, WM8960_LOUT1, l);
	if (ret < 0) {
		return ret;
	}
	ret = wm8960_reg_write(dev, WM8960_ROUT1, r);
	if (ret < 0) {
		return ret;
	}

	/* Speaker volume */
	ret = wm8960_reg_write(dev, WM8960_LOUT2, l);
	if (ret < 0) {
		return ret;
	}
	ret = wm8960_reg_write(dev, WM8960_ROUT2, r);
	if (ret < 0) {
		return ret;
	}
...
static int wm8960_validate_dai_cfg(struct audio_codec_cfg *cfg)
{
	...
	if (cfg->dai_cfg.i2s.channels == 0 ||
	    cfg->dai_cfg.i2s.channels > 2) {
		LOG_ERR("Unsupported channel count: %d",
...
static int wm8960_update_mclk(const struct device *dev,
			      struct audio_codec_cfg *cfg)
{
	const struct wm8960_config *config = dev->config;
	struct wm8960_data *data = dev->data;
	...
	err = clock_control_on(config->mclk_dev, config->mclk_name);
	if (err < 0) {
...
static int wm8960_apply_clocking(const struct device *dev,
				 struct audio_codec_cfg *cfg)
{
	struct wm8960_data *data = dev->data;
	int ret;
	...
	if (data->use_pll) {
		ret = wm8960_reg_update(dev, WM8960_CLOCK1, 0x0001, 0x0001);
		if (ret < 0) {
			return ret;
		}
		...
		ret = wm8960_reg_update(dev, WM8960_POWER2,
				  WM8960_POWER2_PLL_EN, 0);
		if (ret < 0) {
			return ret;
		}

		ret = wm8960_reg_update(dev, WM8960_CLOCK1, 0x0001, 0x0000);
		if (ret < 0) {
			return ret;
		}
	}
...
static int wm8960_set_default_volumes(const struct device *dev)
{
	struct wm8960_data *data = dev->data;
	int ret;

	if (data->route == AUDIO_ROUTE_BYPASS ||
	    data->route == AUDIO_ROUTE_CAPTURE ||
	    data->route == AUDIO_ROUTE_PLAYBACK_CAPTURE) {
		ret = wm8960_set_input_volume(dev, WM8960_DEFAULT_IN_VOLUME,
					     WM8960_DEFAULT_IN_VOLUME,
					     true);
		if (ret < 0) {
	...
	if (data->route == AUDIO_ROUTE_BYPASS ||
	    ...
	    data->route == AUDIO_ROUTE_PLAYBACK_CAPTURE) {
		ret = wm8960_set_output_volume(dev, WM8960_DEFAULT_OUT_VOLUME,
					      WM8960_DEFAULT_OUT_VOLUME,
					      true);
		if (ret < 0) {
			LOG_ERR("Failed to set default output volume: %d", ret);
			return ret;
		}
	}
...
static int wm8960_configure_jack_detect(const struct device *dev)
{
	const struct wm8960_config *config = dev->config;
	int ret;

	if (config->gpio1_sel == WM8960_TEMP_OK) {
		ret = wm8960_reg_update(dev, WM8960_ADDCTL4,
				  WM8960_ADDCTL4_TSENSEN,
				  WM8960_ADDCTL4_TSENSEN);
		if (ret < 0) {
			return ret;
		}
...
static int wm8960_apply_properties(const struct device *dev)
{
	...
	/* Left input volume */
	ret = wm8960_reg_update(dev, WM8960_LINVOL, WM8960_LINVOL_IPVU, WM8960_LINVOL_IPVU);
	if (ret < 0) {
		return ret;
	}

	/* Right input volume */
	ret = wm8960_reg_update(dev, WM8960_RINVOL, WM8960_RINVOL_IPVU, WM8960_RINVOL_IPVU);
	if (ret < 0) {
		return ret;
	}

	/* Left ADC gain */
	ret = wm8960_reg_update(dev, WM8960_LADC, WM8960_LADC_ADCVU, WM8960_LADC_ADCVU);
	if (ret < 0) {
		return ret;
	}

	/* Right ADC gain */
	ret = wm8960_reg_update(dev, WM8960_RADC, WM8960_RADC_ADCVU, WM8960_RADC_ADCVU);
	if (ret < 0) {
		return ret;
	}

	/* Headphone left output volume */
	ret = wm8960_reg_update(dev, WM8960_LOUT1, WM8960_LOUT1_OUT1VU, WM8960_LOUT1_OUT1VU);
	if (ret < 0) {
		return ret;
	}

	/* Headphone right output volume */
	ret = wm8960_reg_update(dev, WM8960_ROUT1, WM8960_ROUT1_OUT1VU, WM8960_ROUT1_OUT1VU);
	if (ret < 0) {
		return ret;
	}

	/* Class-D left output volume */
	ret = wm8960_reg_update(dev, WM8960_LOUT2, WM8960_LOUT2_SPKVU, WM8960_LOUT2_SPKVU);
	if (ret < 0) {
		return ret;
	}

	/* Class-D right output volume */
	ret = wm8960_reg_update(dev, WM8960_ROUT2, WM8960_ROUT2_SPKVU, WM8960_ROUT2_SPKVU);
	if (ret < 0) {
		return ret;
	}

	/* Left DAC gain */
	ret = wm8960_reg_update(dev, WM8960_LDAC, WM8960_LDAC_DACVU, WM8960_LDAC_DACVU);
	if (ret < 0) {
		return ret;
	}

	/* Right DAC gain */
	ret = wm8960_reg_update(dev, WM8960_RDAC, WM8960_RDAC_DACVU, WM8960_RDAC_DACVU);
	if (ret < 0) {
		return ret;
	}

	/* Clear anti-pop circuitry */
...
static int wm8960_configure(const struct device *dev,
			    struct audio_codec_cfg *cfg)
{
	struct wm8960_data *data = dev->data;
	int ret;
					 ...
					 audio_property_value_t val)
{
	...
	switch (channel) {
	case AUDIO_CHANNEL_FRONT_LEFT:
		return wm8960_set_input_volume(dev,
...
static int wm8960_set_output_mute_property(const struct device *dev,
					  audio_channel_t channel,
					  audio_property_value_t val)
{
	struct wm8960_data *data = dev->data;
	...
	switch (channel) {
	case AUDIO_CHANNEL_FRONT_LEFT:
		return wm8960_set_output_volume(dev,
...
static int wm8960_set_input_volume_property(const struct device *dev,
					   audio_channel_t channel,
					   audio_property_value_t val)
{
	struct wm8960_data *data = dev->data;
	...
	switch (channel) {
	case AUDIO_CHANNEL_FRONT_LEFT:
		return wm8960_set_input_volume(dev, val.vol,
					      data->input_volume_right,
					      true);
	case AUDIO_CHANNEL_FRONT_RIGHT:
		return wm8960_set_input_volume(dev, data->input_volume_left,
					      val.vol, true);
	case AUDIO_CHANNEL_ALL:
		return wm8960_set_input_volume(dev, val.vol, val.vol, true);
	default:
		return -EINVAL;
	}
}

static int wm8960_set_output_volume_property(const struct device *dev,
					    audio_channel_t channel,
					    audio_property_value_t val)
{
	struct wm8960_data *data = dev->data;
	...
	switch (channel) {
	case AUDIO_CHANNEL_FRONT_LEFT:
		return wm8960_set_output_volume(dev, val.vol,
					       data->output_volume_right,
					       true);
	case AUDIO_CHANNEL_FRONT_RIGHT:
		return wm8960_set_output_volume(dev, data->output_volume_left,
					       val.vol, true);
	case AUDIO_CHANNEL_ALL:
		return wm8960_set_output_volume(dev, val.vol, val.vol, true);
	default:
		return -EINVAL;
	}
}

static int wm8960_set_property(const struct device *dev,
			       ...
			       audio_property_value_t val)
{
	...
	case AUDIO_PROPERTY_INPUT_MUTE:
		return wm8960_set_input_mute_property(dev, channel, val);
	case AUDIO_PROPERTY_OUTPUT_MUTE:
		return wm8960_set_output_mute_property(dev, channel, val);
	case AUDIO_PROPERTY_INPUT_VOLUME:
		return wm8960_set_input_volume_property(dev, channel, val);
	case AUDIO_PROPERTY_OUTPUT_VOLUME:
		return wm8960_set_output_volume_property(dev, channel, val);
	default:
		return -EINVAL;
	}
}
...
static int wm8960_init(const struct device *dev)
{
	...
	data->output_volume_left = WM8960_DEFAULT_OUT_VOLUME;
	data->output_volume_right = WM8960_DEFAULT_OUT_VOLUME;
	data->muted = false;

	/* Initialize input-related settings */
	data->input_volume_left = WM8960_DEFAULT_IN_VOLUME;
	data->input_volume_right = WM8960_DEFAULT_IN_VOLUME;
...
#define WM8960_INIT(inst) \
	...
	static const struct wm8960_config wm8960_config_##inst = { \
		...
		.mclk_name = (clock_control_subsys_t)DT_INST_PHA_BY_NAME_OR( \
			inst, \
			clocks, \
			mclk, \
			name, \
